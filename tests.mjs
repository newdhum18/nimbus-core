...
const tests=[
['version',worker.includes('35.2.0-free-plan-safe')],['catalog 300',catalog.length===300],['enabled 80',catalog.filter(x=>x.enabled).length===80],['unique ids',new Set(catalog.map(x=>x.id)).size===300],
...
console.log(JSON.stringify({ok:true,tests:tests.length,catalog:catalog.length,enabled:catalog.filter(x=>x.enabled).length,version:'35.2.0-free-plan-safe'}));
