...
export async function route(request, env) {
  ...
  try {
    ...
    if (url.pathname === "/api/results/recovery" && request.method === "GET") {
      ...
      const rows=await db.prepare(`SELECT er.id,er.host,er.url,er.normalized_url,er.reason,er.status,er.http_status,er.content_type,er.dynamic,er.encoded_target,er.embed_detected,er.markers_json,er.evidence_json,er.last_seen_at,s.name source_name FROM extraction_recovery er LEFT JOIN sources s ON s.id=er.source_id WHERE er.status='open' ORDER BY er.dynamic DESC,er.encoded_target DESC,er.last_seen_at DESC LIMIT ?`).bind(limit).all().catch(()=>({results:[]}));
      const items=(rows.results||[]).map(r=>({id:r.id,host:r.host,url:r.url,root_url:r.url,state:r.status,source_name:r.source_name,reason:r.reason,http_status:Number(r.http_status||0),content_type:r.content_type,dynamic:Boolean(r.dynamic),encoded_target:Boolean(r.encoded_target),embed_detected:Boolean(r.embed_detected),markers:JSON.parse(r.markers_json||"[]"),evidence:JSON.parse(r.evidence_json||"{}"),last_seen_at:r.last_seen_at}));
      return ok({total:items.length,blocked:items.filter(x=>[401,403,429].includes(x.http_status)).length,dynamic:items.filter(x=>x.dynamic).length,encoded:items.filter(x=>x.encoded_target).length,items},200,cors);
    }
