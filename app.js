...
async function runScan(more){ $('scanMessage').className='notice show'; $('scanMessage').textContent='Scanning...'; $('scanOut').textContent=''; $('scanList').className='list empty'; $('scanList').textContent='Scanning...'; $('scanSummary').innerHTML=''; const d=await api('/api/search',{method:'POST',body:JSON.stringify({mode:'auto',continueScan:!!more,includeAdult:!!document.getElementById('adultPublicToggle')?.checked,batchSize:8})}); $('scanOut').textContent=pretty(d); if(d.ok){ $('scanMessage').textContent=`Auto Scan finished: ${d.summary?.links_found||0} links found, ${d.summary?.new_links||0} new.`; renderSummary(d.summary); renderLinks($('scanList'),d.saved||[]); loadDashboard(); } else { $('scanMessage').textContent=d.message||d.error||'Scan failed'; $('scanList').className='list empty'; $('scanList').textContent=d.message||d.error||'Scan failed'; } }
