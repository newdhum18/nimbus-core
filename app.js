...
async function runScan(keyword="", opts={}){
  ...
  try{
    const data=await api("/api/search",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({q:keyword,countries,limitPages:140})});
    const summaryText=[
      `OK: ${data.ok}`,
      `Mode: ${data.mode}`,
      `Query: ${data.query}`,
      ...
      `Engines: ${JSON.stringify(data.engine_stats||{})}`
    ].join("\n");
